...
additional_param_combos = () # add in different parameter combos here
...
for((i=0;i<OPTNUM;i++)); do
  ...
  mesh_fol='/home/joshua/simulations/scale/mewtwo_script_'"$option"'.mlx'

  for ((num=0;num<NUMTIME;num++)); do
    # create file and part names

    timestep=${time_array[num]}
    err_bool='python err_search.py $timestep'
